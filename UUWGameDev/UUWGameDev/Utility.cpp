...
namespace
{
	...
	{
		auto oSeed = static_cast<unsigned long>(std::time(nullptr));
		return std::default_random_engine(oSeed);
	}

	auto oRandomEngine = CreateRandomEngine();
}
...
void CenterOrigin(sf::Sprite& pSprite)
{
	...
	pSprite.setOrigin(std::floor(bounds.left + bounds.width / 2.f), std::floor(bounds.top + bounds.height / 2.f));
}

void CenterOrigin(sf::Text& pText)
{
	sf::FloatRect bounds = pText.getLocalBounds();
	pText.setOrigin(std::floor(bounds.left + bounds.width / 2.f), std::floor(bounds.top + bounds.height / 2.f));
}

float toDegree(float pRadian)
{
	return 180.f / 3.141592653589793238462643383f * pRadian;
}

float toRadian(float pDegree)
{
	return 3.141592653589793238462643383f / 180.f * pDegree;
}

int randomInt(int pExclusiveMax)
{
	std::uniform_int_distribution<> distr(0, pExclusiveMax - 1);
	return distr(oRandomEngine);
}

float length(sf::Vector2f pVector)
{
	return std::sqrt(pVector.x * pVector.x + pVector.y * pVector.y);
}

sf::Vector2f unitVector(sf::Vector2f pVector)
{
	assert(pVector != sf::Vector2f(0.f, 0.f));
	return pVector / length(pVector);
}
